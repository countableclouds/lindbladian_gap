...
class CyclicGraph:
    ...
    def adj_matrix(n):
        return [[1 if abs(i-j) in [1, n-1] else 0 for j in range(n)] for i in range(n)]
    ...
    def jumps_diag_trans(self, a, b, l, m): # returns the ((a,b), (l,m)) jump coefficient of the Lindbladian, using jumps to adjacent vertices
        n = self.n
    ...
    def jumps_adjacent_trans(self, a, b, l, m): # returns the ((a,b), (l,m)) jump coefficient of the Lindbladian, using jumps to adjacent vertices
        n = self.n
...
class PathGraph:
    def __init__(self, n, jumps):
        ...
        self.energies = np.array([2  * np.cos(np.pi * i/(n+1)) for i in range(1, n+1)])
    ...
    def first_sum(self, j, k, l, m):# computes the sum from i = 1 to n of sin(ij*pi/(n+1))*sin(ik*pi/(n+1))*sin(il*pi/(n+1))*sin(im*pi/(n+1))
        n= self.n
        total = 0
        j_ = j+1
        k_ = k+1
    ...
    def first_sum_test(self, j, k, l, m):
        ...
        out = round(16/(n+1)*sum([h(n, i, j_) * h(n, i, k_)*h(n, i, l_)*h(n, i, m_) for i in range(1, n+1)])) *(n+1)/16
        assert out == self.first_sum(j, k, l, m)

    def second_sum(self, j, k, l): # computes the sum from i = 1 to n of sin(ij*pi/(n+1))^2*sin(ik*pi/(n+1))*sin(il*pi/(n+1))
        n= self.n
        j_ = j+1
        k_ = k+1
        l_ = l+1
    ...
    def second_sum_test(self, j, k, l):
        ...
        n = self.n
        h = lambda n, o, p: np.sin(np.pi * o * p/(n+1))
        out = round(16/(n+1)*sum([h(n, i, j_)**2 * h(n, i, k_)*h(n, i, l_) for i in range(1, n+1)])) *(n+1)/16
        assert out == self.second_sum(j, k, l), (out, self.second_sum(j, k, l), j, k, l)
        
    def jumps_diag_trans(self, a, b, l, m): # returns the ((a,b), (l,m)) jump coefficient of the Lindbladian, using jumps |i><i|
        coeff = self.first_sum(a, b, l, m)*(2/(n+1))**2
        return coeff

    def jumps_diag_decay(self, a, l, j): # returns the ((a,b), (l,m)) jump coefficient of the Lindbladian, using jumps |i><i|
        coeff = self.second_sum(j, a, l)*(2/(n+1))**2
        return coeff
